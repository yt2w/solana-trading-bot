...
class BotConfig:
    ...
    def validate(self) -> list[str]:
        ...
        if not self.wallet.is_configured():
            issues.append("WARNING: Wallet private key not configured - trading disabled")
        
        if self.trading.paper_trading:
            issues.append("INFO: Paper trading mode enabled - no real transactions")
...
if __name__ == "__main__":
    ...
    print(f"Network: {config.solana.network}")
    print(f"RPC URL: {config.solana.rpc_url}")
