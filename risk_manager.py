...
class TakeProfitLevel:
    ...
    price_pct: Decimal  # % above entry price
    sell_pct: Decimal   # % of position to sell
...
class StopLoss:
    ...
    stop_type: StopType
    trigger_pct: Decimal  # % below entry (or trailing high)
    trailing_high: Optional[Decimal] = None  # Highest price seen (for trailing)
...
class Position:
    ...
    entry_value_sol: Decimal  # Value at entry in SOL
    entry_time: datetime
...
class CircuitBreaker:
    ...
    def __init__(
        ...
        recovery_timeout: int = 30,  # minutes
...
class PositionManager:
    ...
    def __init__(self):
        ...
        self.user_positions: Dict[int, Set[str]] = defaultdict(set)  # user_id -> position_ids
        self.token_positions: Dict[str, Set[str]] = defaultdict(set)  # token_address -> position_ids
        self._lock = asyncio.Lock()
...
class RiskManager:
    ...
    async def _validate_buy(
        ...
        # 6. Check if already have position in this token
